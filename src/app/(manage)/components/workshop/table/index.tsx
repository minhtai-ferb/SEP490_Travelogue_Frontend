...
function WorkshopTable({ href }: { href: string }) {
  ...
  const fetchModeratorWorkshops = useCallback(async () => {
    try {
      ...
      toast.error("Lỗi khi lấy danh sách trải nghiệm làng nghề");
    }
  ...
  return (
    <div className="p-4">
      <Card>
        <CardHeader>
          <CardTitle>Danh sách trải nghiệm làng nghề</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <Select
              value={String(status)}
              onValueChange={(v) => setStatus(v === "all" ? "all" : Number(v))}
            >
              <SelectTrigger>
            ...
            <Input
              placeholder="CraftVillageId"
              value={craftVillageId}
              onChange={(e) => setCraftVillageId(e.target.value)}
            />
            <div className="flex gap-2">
              <Button onClick={fetchModeratorWorkshops} disabled={loading}>
                Lọc
              </Button>
              <Button
                variant="outline"
          ...
          <WorkshopDataTable workshops={workshops} loading={loading} href={href} />
        </CardContent>
      </Card>
    </div>
  );
