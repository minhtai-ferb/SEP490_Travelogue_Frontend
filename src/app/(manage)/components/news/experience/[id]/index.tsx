...
export default function ViewExperience({ href }: { href: string }) {
  const router = useRouter();
  const params = useParams();
  const { getNewsById, deleteNews, loading } = useNews();
  const [experienceItem, setExperienceItem] = useState<ExperienceData | null>(
    null
  );
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  {getTypeExperienceLabel(experienceItem.typeExperience || 0) ??
                    "Không xác định"}
                </span>
              </div>
            </CardContent>
                  ...
                  {experienceItem.medias.map((media, index) => (
                    <div key={index} className="relative">
                      <img
                        src={
                          media.mediaUrl ||
                          "/placeholder.svg?height=192&width=256&query=placeholder image"
                        }
