...
import { Eye, XCircle, Wallet, User, MapPin } from "lucide-react";
...
export interface BookingTableProps {
  ...
  onViewTripPlan?: (record: BookingItem) => void;
...
export function BookingTableComponent({
  ...
  const columns = [
        ...
        <div className="space-y-1">
          {r.tripPlanName && (
            <div>
              ...
              <div className="text-sm mt-1">{r.tripPlanName}</div>
              {r.tripPlanId && onViewTripPlan && (
                <AntButton
                  type="link"
                  size="small"
                  onClick={() => onViewTripPlan(r)}
                  className="p-0 h-auto text-xs"
                >
                  Xem chi tiết
                </AntButton>
          ...
          {!r.tripPlanName && r.tourGuideName && (
    ...
    },
    {
      ...
      key: "status",
      width: 150,
      render: (_: any, r: BookingItem) => getStatusTag(r),
      filters: [
