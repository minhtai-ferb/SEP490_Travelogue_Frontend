...
export const getActivityIcon = (activityType: number) => {
  switch (activityType) {
    case 1:
      return <Camera className="h-4 w-4" />; // Tham quan
    case 2:
      return <Utensils className="h-4 w-4" />; // Ăn uống
    case 3:
      return <Hammer className="h-4 w-4" />; // Trải nghiệm làng nghề
    case 4:
      return <Coffee className="h-4 w-4" />; // Nghỉ ngơi
    case 5:
      return <ShoppingBag className="h-4 w-4" />; // Mua sắm
    case 6:
      return <Gamepad2 className="h-4 w-4" />; // Giải trí
    case 7:
      return <Activity className="h-4 w-4" />; // Trải nghiệm
    default:
              ...
              <div className="flex items-center gap-6 text-white/80">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">
                    {tour.finalPrice.toLocaleString("vi-VN")} VNĐ
                  </span>
                </div>
              </div>
            </div>
            ...
            {tour.medias && tour.medias.length > 0 && (
              <div className="lg:w-80">
                <div className="relative rounded-lg overflow-hidden shadow-2xl">
                  <img
                    src={
                      tour.medias.find((m) => m.isThumbnail)?.mediaUrl ||
                      tour.medias[0].mediaUrl
                    }
                    alt={tour.name}
              ...
              <Accordion type="multiple" className="space-y-3">
                {tour.days.map((day, dayIndex) => (
                  <AccordionItem
                    ...
                    className="border-2 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200"
                  >
                        ...
                        <div className="text-right text-sm text-gray-500">
                          <div className="flex items-center gap-1">
                            ...
                            {day.activities.length > 0 && (
                              <span>
                                {day.activities[0].startTimeFormatted} -{" "}
                                {
                                    ...
                                    <div className="flex sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                          ...
                                          <Badge
                                            className={`border ${getActivityColor(
                                              activityExt.activityType || 0
                                            )}`}
                                          >
                                            {getActivityIcon(
                                        ...
                                        {activityExt.travelTimeFromPrev > 0 && (
                                          <div className="flex items-center gap-2 p-3 bg-white/50 rounded-lg">
                                            ...
                                            <div>
                                              ...
                                              <p className="font-semibold">
                                                {activityExt.travelTimeFromPrev}{" "}
                                                phút
                                              </p>
                                              <p className="text-xs text-gray-600">
                                                {activityExt.distanceFromPrev}{" "}
                                                km
                                              </p>
                                            </div>
                                          </div>
                ...
                <Card className="sticky shadow-md mt-4">
                  ...
                  <CardContent className="space-y-3">
                    {tour.schedules.map((schedule, index) => (
                      <div
                        key={index}
                        className="group relative p-4 border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-md transition-all duration-200 bg-gradient-to-r from-white to-blue-50/30"
                      >
                        {/* Main Schedule Info */}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="font-medium text-gray-900">
                                {(schedule.tourGuide as any)?.userName ||
                                  "Chưa phân công"}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="font-medium text-gray-900 truncate">
                                {(schedule.tourGuide as any)?.email ||
                                  "Chưa có email"}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex justify-end">
                          <Button
                            variant="outline"
                            size="sm"
                            className="group-hover:bg-blue-50 group-hover:border-blue-300 group-hover:text-blue-700 transition-all duration-200 font-medium"
                            onClick={() => {
                              router.push(
                                `/admin/tour/${tour.tourId}/tour-schedule/${schedule.scheduleId}`
                              );
                            }}
                          >
                            <div className="flex items-center gap-2">
