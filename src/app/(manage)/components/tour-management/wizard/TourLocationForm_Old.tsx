...
import type { Workshop } from "@/types/tour-location"
...
export function TourLocationForm({
	...
	const handleLocationSelect = async (locationId: string) => {
		...
		try {
			...
			if (locationData.craftVillage?.workshopsAvailable && locationData.craftVillage.workshop) {
				...
				// Step 2: Regular location - let user input start/end time manually
				setShowWorkshopDialog(false)
				calculateTravelTime(locationData)
			}
		} catch (error) {
	...
	const handleLocationSelect = async (locationId: string) => {
		...
		if (location?.craftVillage?.workshopsAvailable && location.craftVillage.workshop) {
			...
			// Set workshopId to the actual workshop ID
			setNewLocation(prev => ({
				...prev,
				workshopId: location?.id
			}))
	...
	const handleSessionSelect = (session: WorkshopSession | null) => {
		...
		// Update workshop data with session information
		setNewLocation(prev => ({
			...prev,
			workshopSessionRuleId: session.id,
			preferredStartTime: session.startTime,
			preferredEndTime: session.endTime
		}))
	...
	const handleTicketSelect = (ticketId: string) => {
		...
		// Update workshop ticket type ID
		setNewLocation(prev => ({
			...prev,
		...
		// Check if it's a "tham quan" (visit only) ticket
		const isVisitOnlyTicket = selectedTicket?.name?.toLowerCase().includes('tham quan')

		if (isVisitOnlyTicket && selectedTicket) {
			// Auto-fill times for visit-only tickets
	...
	const timeStringToTicks = (timeString: string): number => {
		...
		return totalSeconds * 10000000 // .NET ticks = seconds * 10,000,000
	}
	...
	const handleAddLocation = async () => {
		...
		const itemToAdd: TourLocationBulkRequest = {
			...
			estimatedStartTime: normalizeTimeString(newLocation.startTime),
			estimatedEndTime: normalizeTimeString(newLocation.endTime),
			activityType: newLocation.activityType || 1
		}
	...
	const handleSubmit = () => {
		...
		const payload = locations.map((loc) => ({
			tourPlanLocationId: undefined, // Will be generated by backend
			locationId: loc.locationId,
			dayOrder: loc.dayOrder,
			activityType: loc.activityType || 1,
			startTime: loc.startTime,
			endTime: loc.endTime,
			notes: loc.notes || "",
			travelTimeFromPrev: loc.travelTimeFromPrev || 0,
			distanceFromPrev: loc.distanceFromPrev || 0,
			estimatedStartTime: loc.estimatedStartTime && loc.estimatedStartTime !== "00:00:00"
				? loc.estimatedStartTime
				: loc.startTime,
			estimatedEndTime: loc.estimatedEndTime && loc.estimatedEndTime !== "00:00:00"
				? loc.estimatedEndTime
				: loc.endTime,
			// Workshop fields (only if workshop is selected)
			...(loc.workshopId && {
				workshopId: loc.workshopId,
				workshopTicketTypeId: loc.workshopTicketTypeId,
				workshopSessionRuleId: loc.workshopSessionRuleId,
				preferredStartTime: loc.preferredStartTime,
				preferredEndTime: loc.preferredEndTime
			})
		}))
								...
								<div className="flex items-start gap-3">
									...
									<div className="flex-1">
										<h4 className="font-bold text-purple-900 mb-2">
											Thông tin làng nghề: {selectedLocation.craftVillage.workshop.name}
										</h4>
										<div className="space-y-2 text-sm">
											...
											<p className="text-purple-700">
												<strong>📝 Mô tả:</strong> {selectedLocation.craftVillage.workshop.description}
											</p>
											<p className="text-purple-700">
												<strong>⏰ Thời gian hoạt động:</strong> {formatTime(selectedLocation.openTime || "")} - {formatTime(selectedLocation.closeTime || "")}
											</p>
											<div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
												...
												<p className="text-blue-700 text-xs mt-1">
													Vì đây là làng nghề nên sẽ có các trải nghiệm workshop trong này.
													Các ngày trong tuần được tính: 0 = Chủ nhật, 1 = Thứ hai, 2 = Thứ ba...
													Vui lòng chọn loại vé và session phù hợp.
												</p>
											</div>
										</div>
									</div>
