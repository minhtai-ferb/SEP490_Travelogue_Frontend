...
export function TourLocationForm({
	...
	const handleTicketSelect = (ticketId: string) => {
		...
		// Check if it's a combo ticket (experience ticket)
		if (ticket.isCombo) {
	...
	const handleSessionSelect = (sessionId: string) => {
		...
		setNewLocation(prev => ({
			...
			preferredStartTime: {
				ticks: timeStringToTicks(startTime)
			},
			preferredEndTime: {
				ticks: timeStringToTicks(endTime)
			}
		}))
	}
	...
	const handleWorkshopConfirm = () => {
		...
		if (selectedTicketType.isCombo && !selectedSession) {
	...
	const fetchRouteMetrics = async (from: { lat: number; lng: number }, to: { lat: number; lng: number }) => {
		try {
			const response = await axios.get(VIETMAP_ROUTE_ENDPOINT, {
				params: {
					api_key: SeccretKey.VIETMAP_API_KEY,
					...
					<div className="grid grid-cols-2 gap-4">
						<div>
							...
							<LocationSelect
								...
								selectedLocationId={newLocation.locationId}
										...
										<SelectItem key={ticket.id} value={ticket.id}>
											<div className="flex items-center justify-between w-full">
												...
												<Badge variant={ticket.isCombo ? "default" : "secondary"}>
													{ticket.isCombo ? "Trải nghiệm" : "Tham quan"}
												</Badge>
											</div>
										</SelectItem>
									))}
